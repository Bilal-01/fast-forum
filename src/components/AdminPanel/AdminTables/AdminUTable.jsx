...
//     {auth.user? stud=auth.user.id:stud=''}
  ...
  function deleteEntry(Rid){
    let id= Rid
    console.log(id)
    axios.delete('http://localhost/forum/php/index.php', {data: id}).then(function(response){
        ...
        <TableBody>
          {rows.map((row, i) => (
            ...
            >
              ...
              <StyledTableCell align="center"><Button sx={{background:'#e25a44'}}variant="contained" onClick={(event)=>deleteEntry(row.carpool_id)}>Delete</Button></StyledTableCell>
            </StyledTableRow>
          ))}
        </TableBody>
      </Table>
